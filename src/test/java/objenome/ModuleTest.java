...
public final class ModuleTest {
  ...
  @Test public void childModuleWithManualConstruction() {
    ...
    O objectGraph = O.load(new DynamicLoader(), new ModuleWithConstructor("a"), new TestModule());
    TestEntryPoint entryPoint = new TestEntryPoint();
    objectGraph.with(entryPoint);
  ...
  @the(in = A.class) public static class TestModuleA {
    @out
    static A a() { return new A(); }
  }

  @the(extend = TestModuleA.class, in = B.class) public static class TestModuleB {}

  @Test public void autoInstantiationOfModules() {
    // Have to make these non-method-scoped or instantiation errors occur.
    O objectGraph = O.load(new DynamicLoader(), TestModuleA.class);
    assertThat(objectGraph.a(A.class)).isNotNull();
  }

  @Test public void autoInstantiationOfIncludedModules() {
    // Have to make these non-method-scoped or instantiation errors occur.
    O objectGraph = O.load(new DynamicLoader(), new TestModuleB()); // TestModuleA auto-created.
    assertThat(objectGraph.a(A.class)).isNotNull();
  ...
  public void childModuleMissingModuleAnnotation() {
    try {
      O.load(new DynamicLoader(), new ChildModuleMissingModuleAnnotation());
  ...
  @Test public void moduleExtendingClassThrowsException() {
    try {
      O.load(new DynamicLoader(), new ThreadModule());
      fail();
    } catch (IllegalArgumentException e) {
  ...
  @Test public void provideProviderFails() {
    ...
    try {
      O.load(new DynamicLoader(), new ProvidesProviderModule());
      fail();
  ...
  @Test public void provideRawProviderFails() {
    ...
    try {
      O.load(new DynamicLoader(), new ProvidesRawProviderModule());
      fail();
    } catch (IllegalStateException e) {
      assertThat(e.getMessage()).startsWith("@Provides method must not return Provider directly: ");
  ...
  @Test public void provideLazyFails() {
    ...
    try {
      O.load(new DynamicLoader(), new ProvidesLazyModule());
      fail();
    } catch (IllegalStateException e) {
  ...
  @Test public void provideRawLazyFails() {
    ...
    try {
      O.load(new DynamicLoader(), new ProvidesRawLazyModule());
      fail();
    } catch (IllegalStateException e) {
      assertThat(e.getMessage()).startsWith("@Provides method must not return Lazy directly: ");
